...
server.createPhantom = function() {
    ...
    //The default value is --ssl-protocol=tlsv1 but was not working with out HAProxy SSL config
    //take in account that we are not using this args, we are specifying PHANTOM_ARGS in the prerender-service.erb
...
server.onRequest = function(req, res) {
    ...
    if (req.prerender.url=="ping") {
        util.log('Service HealthCheck');
        res.end("pong");
        return res;
    }
